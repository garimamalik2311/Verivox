...
class DualStreamFusionClassifier:
    ...
    def _load_artifacts(self):
        ...
        model_path = (
            ...
            else os.path.join(base_dir, self.MODEL_PATH)
        )

        scaler_path = (
            self.SCALER_PATH
            if os.path.exists(self.SCALER_PATH)
            else os.path.join(base_dir, self.SCALER_PATH)
        )

        config_path = (
            self.CONFIG_PATH
            if os.path.exists(self.CONFIG_PATH)
            else os.path.join(base_dir, self.CONFIG_PATH)
        )

        if not os.path.exists(model_path) or not os.path.exists(scaler_path):
            print(
                f"[AcousticModel] Warning: Model weights not found at "
                f"{model_path}"
            )
            return
        ...
        self.model = GatedDualStreamFusion(
            ssl_dim=ssl_dim,
            dsp_dim=dsp_dim,
            proj_dim=proj_dim
        )
        ...
        if _TRANSFORMERS_AVAILABLE:
            try:
                ...
                print(
                    f"[AcousticModel] Could not load "
                    f"Hugging Face backbone: {e}"
                )
    ...
    def predict(
        ...
        if self.model is None:
            raise RuntimeError(
                "Dual-stream acoustic model is not loaded. "
                "Check reports/fused_acoustic_model.pt and "
                "reports/fused_acoustic_scaler.joblib."
            )
        ...
        if self.mms_model is not None and self.mms_fe is not None:
            ...
            with torch.no_grad():
                ...
                ssl_tensor = torch.mean(
                    out,
                    dim=1
                )

        else:
            # Fallback zero-latent if transformers are unavailable
            # or the MMS backbone could not be loaded.
            ssl_tensor = torch.zeros(
                (1, 1024),
                dtype=torch.float32
            ).to(self.device)
